...
require_once '../Controller/NoteMyController.php';
...
$user = $_SESSION['user'];
$noteController = new NoteMyController();
...
$totalNotas = $noteController->getTotalNotas($user['id']); // Número total de notas del usuario
$totalPages = ceil($totalNotas / $limit); // Número total de páginas

$notas = $noteController->getNotas($user['id'], $limit, $offset, $orderBy, $orderDir);

if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    if (isset($_POST['update_estado'])) {
        $id = $_POST['nota_id'];
        $estado = $_POST['estado'];
        if ($noteController->updateEstado($id, $estado)) {
...
<head>
    ...
    <style>
        header {
            ...
            /* Para navegadores que soportan el prefijo -webkit- */
            position: sticky;
            top: 0;
            /* Desplazamiento desde la parte superior */
            z-index: 1000;
            /* Asegura que el menú se mantenga por encima de otros elementos */
